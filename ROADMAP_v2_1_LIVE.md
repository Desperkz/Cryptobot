...
- `[x]` P7-19 Параллельные shadow-когорты спорных контекстных фильтров.
  - Причина: текущие revalidation-ветки чаще всего останавливают `RS_NEUTRAL`, `RS_AGAINST`, отсутствие OI, отсутствие ретеста и близкая целевая ликвидность. Ослаблять эти правила сразу в paper нельзя, а последовательный тест одного фильтра занял бы месяцы.
  - Готово: добавлены отдельные virtual buckets с суффиксами `RS_NEUTRAL_SHADOW`, `RS_AGAINST_SHADOW`, `MISSING_OI_SHADOW`, `NO_RETEST_SHADOW`, `NEAR_LIQUIDITY_SHADOW` для применимых источников `SQZ-DYN-UPD`, `TPB` и `MOM`.
  - Готово: virtual-вход создаётся только если снятие ровно одного названного фильтра устраняет единственную причину отказа. Сигналы с двумя и более проваленными gates не попадают ни в одну когорту.
  - Готово: каждая ветка сохраняет исходные TP/SL и остальные контекстные проверки, получает детерминированный source cluster, отдельную статистику и ограничение виртуального риска `0.25%`. Paper/live admission, базовый риск и работа v2.0 не меняются.
  - Готово: dashboard/API маркируют новые строки как `MEASUREMENT_SHADOW`; interim review выполняется после `20`, итоговый human review после `50` закрытых virtual trades. Автоматический перевод в paper/live запрещён.
  - Условие принятия: сравнивать post-cost Avg R, PF, payoff, full-stop rate и DD с revalidation-контролем той же исходной стратегии и того же периода. Не ослаблять production gate по одному PnL или до достижения review milestone.

- `[x]` P7-20 Контроль качества после первой post-epoch paper-сделки и очистка экспериментов.
  - Факт: единственная paper-сделка после epoch `2026-07-26` была `BTCUSDT LONG`, допущенная исключением neutral relative strength; строгий SQZ отклонил бы её. Результат после издержек: `-1.07R` и `-5.19 USDT`.
  - Готово: исключение neutral-RS отключено в paper. Оно продолжает измеряться в отдельных shadow-когортах; строгий SQZ по-прежнему требует aligned relative strength.
  - Факт: `GRID_REVALIDATION` завершил 15 виртуальных сделок с Avg R `-0.234`, PF `0.33` и winrate `26.7%`, превысив условие ранней остановки из P7-18.
  - Готово: `RANGE_GRID` удалён из fresh revalidation cohort. История и базовые diagnostic-сигналы сохранены, paper/live не затронуты.
  - Исправлено: P7-19 раньше наследовал глобальный observe-only режим order-flow, поэтому некоторые сделки ослабляли не один, а несколько gates. Новая когорта `2026-08-07-clean-of` всегда применяет строгий OF ко всем неослабленным правилам.
  - Готово: новые строки получают префиксы `SQZ_DYN_UPD_CLEAN`, `TPB_CLEAN`, `MOM_CLEAN`; прежняя загрязнённая выборка остаётся отдельным историческим артефактом и не смешивается с clean evidence.
  - Ограничение: исторические 48 SQZ-сделок смешивают несколько версий логики. Поэтому по ним не вводится новый универсальный порог breakout ATR; следующий вывод делается минимум после 20 закрытых сделок одной версии и эпохи.

- `[x]` P7-21 Параллельная shadow-лаборатория до context-gates.
  - Причина: clean P7-19 методологически корректен, но за неделю дал только несколько сделок. Почти строгая последовательная фильтрация на research-этапе слишком медленно отвечает, какие именно запреты полезны.
  - Готово: каждый уникальный сгенерированный кандидат `SQZ-DYN-UPD`, `TPB` и `MOM` до context-gates получает детерминированный `source_cluster_id` и одновременно проверяется фиксированной матрицей virtual policies.
  - Готово: матрица содержит `STRICT`, отдельные OF/RS/OI/retest/liquidity arms и заранее определённые пары спорных фильтров. Вариант открывается только если его точный набор relaxed gates устраняет все оставшиеся причины отказа; одиночный arm не поглощает двойной отказ.
  - Готово: строки имеют отдельные имена `SQZ_UPD_LAB_*`, `TPB_LAB_*`, `MOM_LAB_*`, cohort `2026-08-14-parallel-lab`, одинаковый исходный TP/SL, реалистичные комиссии/slippage/funding и virtual risk cap `0.25%`.
  - Готово: повтор одного сигнала в следующих минутных циклах не создаёт новую сделку: дедупликация выполняется по strategy arm и `source_cluster_id`. Arms не конкурируют за portfolio capacity, не влияют на paper/live и не имеют автоматического promotion.
  - Готово: старый P7-19 переведён в historical-only, его данные не удалены. Dashboard/API маркируют одиночные и парные lab arms как `MEASUREMENT_SHADOW`.
  - Ограничение: лаборатория начинает с уже сформированного кандидата стратегии. Внутренние отказы до `strategy.generate()` остаются diagnostics, а не виртуальными сделками; их расширять только отдельным этапом, чтобы не симулировать произвольный рыночный шум.
  - Контроль: на `10` уникальных закрытий arm выполняется ранний review, на `20` сравнение со strict-control по совпадающим source clusters; минимум `50` до обсуждения paper. Нельзя суммировать параллельные arms как портфельный PnL.

- `[x]` P7-22 Условная shadow-карта edge вместо бинарного включения фильтров.
  - Причина: P7-21 показал, что простое снятие одного фильтра часто допускает убыточные входы, но полная последовательность hard-gates оставляет слишком мало сделок. Это означает, что ценность контекста условная, а не одинаковая во всех режимах и направлениях.
  - Готово: каждый сгенерированный кандидат `SQZ-DYN-UPD`, `TPB` и `MOM` до context-gates получает прозрачный score `conditional_context_v1` по regime/direction, order-flow, relative strength, OI, подтверждению входа, целевой ликвидности и объему.
  - Готово: кандидат попадает ровно в один непересекающийся bucket `HIGH`, `MID` или `LOW`; его стабильная context-cell и вклад каждого компонента сохраняются в metadata. Score является исследовательской разметкой, а не прогнозом вероятности и не разрешением на paper/live.
  - Готово: virtual-строки имеют имена `SQZ_UPD_COND_*`, `TPB_COND_*`, `MOM_COND_*`, cohort `2026-08-25-conditional-v1`, исходные TP/SL стратегии, реалистичные издержки и risk cap `0.20%`.
  - Готово: `/conditional-edge` показывает результаты раздельно по source strategy, bucket и context-cell: PnL, total/Avg R, PF, winrate, приближенный 95% CI и размер выборки. Bucket-ветки маркируются `MEASUREMENT_SHADOW` и не имеют automatic promotion.
  - Контроль: первый review на `20` закрытий внутри source+bucket, минимум `50` для решения. Ослаблять paper-фильтры можно только после положительной post-cost evidence одной заранее зафиксированной ячейки/группы, проверки стабильности по символам и human review.
  - Ограничение: нельзя суммировать P7-21 arms и P7-22 buckets как портфельный PnL; это альтернативные виртуальные представления одних и тех же исходных кандидатов.

- `[x]` P7-22A Стабилизация Control API перед следующим исследовательским этапом.
  - Причина: `/strategy-scorecard` загружал в Python всю историю signals/rejections с JSON metadata; на VPS без swap процесс вырастал выше `600 MB` и дважды подтверждённо попадал под OOM, хотя торговый и paper-monitor процессы оставались живы.
  - Готово: абсолютные totals по сигналам и отказам считаются компактными SQL-агрегатами, а confidence/evidence детали строятся только по последним `3000` сигналам и `1500` diagnostics. История trades/shadow_trades и PnL не урезается.
  - Готово: `/conditional-edge` ограничивает загружаемые сделки и context-cell details с явными `truncated`-метками; `/logs` читает только ограниченный хвост файла; dashboard обновляет тяжёлую аналитику раз в `30` секунд, scorecard cache живёт `120` секунд.
