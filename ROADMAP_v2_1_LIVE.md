...
- `[ ]` D-02 Ослаблять или перенастраивать `TREND_FOLLOWING`.
  - Почему отложено: стратегия теперь пишет диагностику, но первые причины отказов показывают `no_trend_structure`, `no_trade_style`, `edge_below_min`; пока нет 24-48 часов свежей статистики.
  - Вернуться к задаче, когда: накопится минимум сутки свежих `STRATEGY_DIAGNOSTIC` по текущему universe после деплоя `ca88eeb`.
