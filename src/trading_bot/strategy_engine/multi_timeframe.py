...
class MultiTimeframeStrategy:
    ...
    def evaluate(
        ...
        if not is_shadow_research and metrics.open_interest_change_pct is None:
            diagnostic.update({"block_reason": "missing_open_interest_confirmation"})
            return None, diagnostic
    ...
    def _direction_with_diagnostic(
        ...
        is_shadow_research = self.config.mode_for_strategy("TREND_FOLLOWING") == "shadow"
        if not is_shadow_research and regime not in {MarketRegime.TREND_UP, MarketRegime.TREND_DOWN}:
            return Direction.NONE, "no_trend_regime", details

        if is_shadow_research and regime not in {MarketRegime.TREND_UP, MarketRegime.TREND_DOWN, MarketRegime.MOMENTUM}:
            return Direction.NONE, "no_trend_regime", details
